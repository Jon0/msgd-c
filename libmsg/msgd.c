...
int msg_init_local(struct msg_client_state *cs, const char *name) {
    ...
    return msg_connection_send_message(&cs->server, 123, msgtype_init_local, msgdata_init_local, (union msgu_any_msg *) &init);
}
...
int msg_peer_connect(struct msg_client_state *cs, const char *addr) {
    ...
    return msg_connection_send_message(&cs->server, 123, msgtype_peer_connect, msgdata_host_addr, (union msgu_any_msg *) &haddr);
}


int msg_list_shares(struct msg_client_state *cs) {
    struct msgu_empty_msg listshare;
    return msg_connection_send_message(&cs->server, 123, msgtype_list_shares, msgdata_empty, (union msgu_any_msg *) &listshare);
}
...
int msg_create_share(struct msg_client_state *cs, char *path) {
    ...
    return msg_connection_send_message(&cs->server, 123, msgtype_add_share_file, msgdata_share_file, (union msgu_any_msg *) &addshare);
}
...
int msg_create_mount(struct msg_client_state *cs, const char *host, const char *share) {
    ...
    return msg_connection_send_message(&cs->server, 123, msgtype_mount, msgdata_mount_node, (union msgu_any_msg *) &mountnode);
}
...
int msg_open_share(struct msg_client_state *cs, char *path) {
    struct msgu_share_file_msg addshare;
    msgu_string_from_static(&addshare.share_name, path);
    return msg_connection_send_message(&cs->server, 123, msgtype_file_open, msgdata_share_file, (union msgu_any_msg *) &addshare);
}
...
int msg_read(struct msg_client_state *cs, int hdl, size_t count) {
    ...
    handle.count = count;
    return msg_connection_send_message(&cs->server, 123, msgtype_file_stream_read, msgdata_node_read, (union msgu_any_msg *) &handle);
}
...
int msg_write(struct msg_client_state *cs, int hdl, const char *buf, size_t count) {
    ...
    return msg_connection_send_message(&cs->server, 123, msgtype_file_stream_write, msgdata_node_write, (union msgu_any_msg *) &stream);
}
